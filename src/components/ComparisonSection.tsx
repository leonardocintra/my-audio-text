...
export default async function ComparisonSection() {
  ...
  const whatsappFailImage = PlaceHolderImages.find(p => p.id === 'whatsapp-fail');
  const ourAppSuccessImage = PlaceHolderImages.find(p => p.id === 'our-app-success');
        ...
        <div className="mt-16 grid grid-cols-1 gap-8 md:grid-cols-2">
          <Card className="border-destructive/50 border-2 bg-card">
            <CardHeader className="flex-row items-center gap-3 space-y-0">
              ...
              <CardTitle className="font-headline text-xl text-destructive">{t('whatsappNative.title')}</CardTitle>
            </CardHeader>
            <CardContent>
              {whatsappFailImage && (
                <Image
                  ...
                  width={600}
                  height={400}
                  className="rounded-lg object-cover aspect-[3/2] w-full border shadow-inner"
                />
              )}
            </CardContent>
          </Card>
          <Card className="border-primary/50 border-2 bg-card">
            <CardHeader className="flex-row items-center gap-3 space-y-0">
              ...
              <CardTitle className="font-headline text-xl text-primary">{t('myAudioText.title')}</CardTitle>
            </CardHeader>
            <CardContent>
              {ourAppSuccessImage && (
                <Image
